...
{
  const auto lhs_idx{op.getInputs().at(ir::operation::Pow::Input::LHS)};
  const auto &lhs = _operands.at(lhs_idx);
  const auto rhs_idx{op.getInputs().at(ir::operation::Pow::Input::RHS)};
...
void DynamicInferer::visit(const ir::operation::Pow &op)
{
  ...
  const auto lhs_ind{op.getInputs().at(ir::operation::Pow::Input::LHS)};
  auto lhs = _tensor_registry->getITensor(lhs_ind);
  auto lhs_shape = getShape(lhs.get());

  const auto rhs_ind{op.getInputs().at(ir::operation::Pow::Input::RHS)};
