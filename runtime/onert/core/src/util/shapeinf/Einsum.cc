...
ir::Shape inferShapeForEinsum(std::string &equation, const std::vector<Shape> &input_shapes)
{
  ...
  ir::Shape newShape(result_shape_dims.size());
  for (size_t i = 0; i < result_shape_dims.size(); ++i)
  {
    newShape.dim(i) = result_shape_dims.at(i);
  }
...
void StaticInferer::visit(const ir::operation::Einsum &op)
{
  ...
  std::vector<nnfw::cker::Shape> shapes;
  for (size_t i = 0; i < inputs.size(); ++i)
  {
    auto input = inputs.at(i);
    auto operand = _operands.at(input);

    if (operand.info().isDynamic())
    {
      output.info().setDynamic();
      return;
    }

    auto shape = operand.info().shape();
    nnfw::cker::Shape tShape(shape.rank());
    for (int k = 0; shape.rank(); ++k)
    {
      tShape.SetDim(k,shape.dim(k));
    }
    shapes.push_back(tShape);
  }

  auto equation = op.param().equation;

  ir::Shape newShape = inferShapeForEinsum(equation, shapes);
  output.info().shape(newShape);
}
...
void DynamicInferer::visit(const ir::operation::Einsum &op)
{
  ...
  {
    auto input_operand = inputs.at(i);
    const auto &input = _tensor_registry->getITensor(input_operand);
    if (input->is_dynamic())
    {
      return;
    }
    auto input_shape = input->getShape();
    nnfw::cker::Shape tShape(input_shape.rank());
    for (int k = 0; input_shape.rank(); ++k)
    {
      tShape.SetDim(k, input_shape.dim(k));
    }
    shapes.push_back(tShape);
  }
  ...
  ir::Shape new_shape = inferShapeForEinsum(equation, shapes);
