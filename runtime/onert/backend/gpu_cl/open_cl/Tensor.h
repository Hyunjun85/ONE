...
public:
  ...
  //   bool WriteData(CLCommandQueue* queue, const TensorFloat32& src);
  //   bool WriteData(
  ...
  //   bool WriteData(CLCommandQueue* queue, const Tensor5DFloat32& src);
  //   bool ReadData(CLCommandQueue* queue, TensorFloat32* dst) const;
  //   bool ReadData(CLCommandQueue* queue, Tensor5DFloat32* dst) const;

  bool CreateFromDescriptor(const TensorDescriptor &desc, CLContext *context);

private:
  bool IsValid(const BHWC &shape) const;
  bool IsValid(const BHWDC &shape) const;

  int GetChannelsAlignment() const;
  int GetAlignedChannels() const;

  // bool WriteDataBHWDC(absl::Span<const float> in,
  //                             CLCommandQueue* queue);
  // bool ReadDataBHWDC(absl::Span<float> out,
