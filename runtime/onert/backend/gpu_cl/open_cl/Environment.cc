...
namespace
{
// bool CreateEnvironment(Environment* result, bool shared,
//                                cl_context_properties egl_context,
//                                cl_context_properties egl_display) {
//   CLDevice gpu;
//   if(CreateDefaultGPUDevice(&gpu) == false) return false;

//   CLContext context;
//   if (shared) {
//     if(CreateCLGLContext(gpu, egl_context, egl_display, &context) == false) return false;
//   } else {
//     if(CreateCLContext(gpu, &context) == false) return false;
//   }
//   CLCommandQueue queue;
//   if(CreateCLCommandQueue(gpu, context, &queue) == false) return false;
//   ProfilingCommandQueue profiling_queue;
//   if(CreateProfilingCommandQueue(gpu, context, &profiling_queue) == false) return false;
...
bool CreateEnvironment(Environment *result)
{
  CLDevice gpu;
  if (CreateDefaultGPUDevice(&gpu) == false)
    return false;

  CLContext context;
  if (CreateCLContext(gpu, &context) == false)
    return false;

  CLCommandQueue queue;
  if (CreateCLCommandQueue(gpu, context, &queue) == false)
    return false;

  ProfilingCommandQueue profiling_queue;
  if (CreateProfilingCommandQueue(gpu, context, &profiling_queue) == false)
    return false;
