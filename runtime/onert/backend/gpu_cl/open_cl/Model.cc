...
{
  return FilterValues([](const ValueDef &v) { return v.value->tensor.is_variable_input; });
}
...
bool GraphFloat32::InsertNodeAfter(NodeId id, Node **new_node)
{
  if (id >= nodes_.size())
  {
    return false;
...
{
  ValueDef def;
  def.value = std::make_unique<Value>(Value{static_cast<ValueId>(values_.size()), {}});
  Value *value = def.value.get();
  values_.push_back(std::move(def));
  return value;
}

bool GraphFloat32::SetProducer(NodeId producer, ValueId value)
{
  ValueDef *v;
  if (LookupValue(value, &v) == false)
    return false;
  Value *value_ptr = v->value.get();
  NodeDef *n;
  if (LookupNode(producer, &n) == false)
    return false;
...
bool GraphFloat32::RemoveProducer(ValueId value)
{
  ValueDef *v;
  if (LookupValue(value, &v) == false)
    return false;
...
bool GraphFloat32::AddConsumer(NodeId consumer, ValueId value)
{
  ValueDef *v;
  if (LookupValue(value, &v) == false)
    return false;
  Value *value_ptr = v->value.get();
  NodeDef *n;
  if (LookupNode(consumer, &n) == false)
    return false;
  Node *node_ptr = n->node.get();
...
bool GraphFloat32::ReplaceInput(NodeId node, ValueId old_value, ValueId new_value)
{
  ValueDef *v_old;
  if (LookupValue(old_value, &v_old) == false)
    return false;
  Value *value_old_ptr = v_old->value.get();
  ValueDef *v_new;
  if (LookupValue(new_value, &v_new) == false)
    return false;
  Value *value_new_ptr = v_new->value.get();
  NodeDef *n;
  if (LookupNode(node, &n) == false)
    return false;
  Node *node_ptr = n->node.get();
  ...
  for (u_int32_t i = 0; i < n->inputs.size(); ++i)
  {
...
bool GraphFloat32::RemoveConsumer(NodeId consumer, ValueId value)
{
  ValueDef *v;
  if (LookupValue(value, &v) == false)
    return false;
...
bool GraphFloat32::DeleteNode(NodeId id)
{
  NodeDef *n;
  if (LookupNode(id, &n) == false)
    return false;
  Node *node_ptr = n->node.get();
...
bool GraphFloat32::DeleteValue(ValueId id)
{
  ValueDef *v;
  if (LookupValue(id, &v) == false)
    return false;
  Value *value_ptr = v->value.get();
  if (v->producer != nullptr)
  {
...
bool GraphFloat32::MakeExactCopy(GraphFloat32 *model) const
{
  model->nodes_.clear();
  ...
  for (auto node_id : execution_plan_)
  {
    ...
    if (node_def.node)
    {
      ...
      {
        if (model->SetProducer(node_def.node->id, output->id) == false)
          return false;
      }
      for (auto input : node_def.inputs)
      {
        if (model->AddConsumer(node_def.node->id, input->id) == false)
          return false;
      }
    }
  }
...
bool GraphFloat32::IsInput(NodeId node, ValueId value)
{
  ...
  return std::find(n.inputs.begin(), n.inputs.end(), v.value.get()) != n.inputs.end();
}

bool GraphFloat32::LookupNode(NodeId id, NodeDef **node_def)
{
  if (id >= nodes_.size())
  {
    return false;
...
bool GraphFloat32::LookupValue(ValueId id, ValueDef **value_def)
{
  if (id >= values_.size())
  {
    return false;
...
bool RemovePrecedingNode(GraphFloat32 *graph, const Node *to_remove, const Node *to_keep)
{
  ...
  for (auto output : graph->FindOutputs(to_remove->id))
  {
    ...
    if (consumers.size() > 1 || (consumers.size() == 1 && consumers[0] != to_keep))
    {
      return false;
    }
  }
  ...
  {
    if (graph->AddConsumer(to_keep->id, input->id) == false)
      return false;
  }
  for (auto output : graph->FindOutputs(to_remove->id))
  {
    if (graph->DeleteValue(output->id) == false)
      return false;
  }
  return graph->DeleteNode(to_remove->id);
}

bool RemoveFollowingNode(GraphFloat32 *graph, const Node *to_remove, const Node *to_keep)
{
  ...
  for (auto input : graph->FindInputs(to_remove->id))
  {
    if (graph->DeleteValue(input->id) == false)
      return false;
  }
  for (auto output : graph->FindOutputs(to_remove->id))
  {
    if (graph->SetProducer(to_keep->id, output->id) == false)
      return false;
  }
  return graph->DeleteNode(to_remove->id);
}

bool RemoveSimpleNodeKeepInput(GraphFloat32 *graph, const Node *simple_node)
{
  ...
  if (graph->DeleteNode(simple_node->id) == false)
    return false;
  for (auto &consumer : consumers)
  {
    if (graph->ReplaceInput(consumer->id, output_id, input_id) == false)
      return false;
  }
  if (graph->DeleteValue(output_id) == false)
    return false;
  if (!producer && consumers.empty())
  {
    if (graph->DeleteValue(input_id) == false)
      return false;
  }
  return true;
}

bool RemoveSimpleNodeKeepOutput(GraphFloat32 *graph, const Node *simple_node)
{
  const auto inputs = graph->FindInputs(simple_node->id);
  const auto outputs = graph->FindOutputs(simple_node->id);
  if (inputs.size() != 1 || outputs.size() != 1)
  ...
  if (producer)
  {
    if (graph->RemoveProducer(input_id) == false)
      return false;
    if (graph->SetProducer(producer->id, output_id) == false)
      return false;
  }
...
bool AddOutput(GraphFloat32 *graph, const Node *from_node, Value **output)
{
  auto link = graph->NewValue();
  if (graph->SetProducer(from_node->id, link->id) == false)
    return false;
  *output = link;
  return true;
}

bool ConnectTwoNodes(GraphFloat32 *graph, const Node *from_node, const Node *to_node,
                     Value **output)
{
  const Node *output_producer = *output ? graph->FindProducer((*output)->id) : nullptr;
  ...
  if (*output)
  {
    if (graph->AddConsumer(to_node->id, (*output)->id) == false)
      return false;
  }
  else
  {
    ...
    Value *link;
    if (AddOutput(graph, from_node, &link) == false)
      return false;
    if (graph->AddConsumer(to_node->id, link->id) == false)
      return false;
    *output = link;
  }
  return true;
