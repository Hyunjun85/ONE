...
public:
  ...
  bool AddOperation(GPUOperation *operation);
  ...
  bool Tune(const TuningParameters &params);

  bool AssembleCode(const DeviceInfo &device_info, CLContext *context);

  bool Compile(const CreationContext &creation_context);

  bool CompileDeserialized(const CreationContext &creation_context);

  virtual bool PostCompileCheck(const DeviceInfo &device_info, const KernelInfo &kernel_info)
  {
    (void)device_info;
    (void)kernel_info;
    return true;
...
protected:
  ...
  virtual bool BindArguments(ArgumentsBinder *args)
  {
    (void)args;
    return true;
  }
