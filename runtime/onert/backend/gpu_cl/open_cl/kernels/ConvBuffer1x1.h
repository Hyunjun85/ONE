...
class ConvBuffer1x1 : public GPUOperation {
 ...
 private:
  ...
  template <DataType T>
  void UploadBiases(const InternalTensor<Linear, T>& biases);
...
template <DataType T>
void ConvBuffer1x1::UploadWeights(const InternalTensor<OHWI, T>& weights) {
...
template <DataType T>
void ConvBuffer1x1::UploadBiases(const InternalTensor<Linear, T>& biases) {
