...
bool ProgramCache::GetOrCreateCLKernel(const std::string &code, const std::string &function_name,
                                       ...
                                       CLKernel *result)
{
  ...
  CLProgram program;
  if (CreateCLProgram(code, options, context, device, &program) == false)
    return false;
  if (result->CreateFromProgram(program, function_name) == false)
    return false;
  ;
...
// bool ProgramCache::AddSerializedCache(
//     const CLContext& context, const CLDevice& device,
...
// bool ProgramCache::GetSerializedCache(
