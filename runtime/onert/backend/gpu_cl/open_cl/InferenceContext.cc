...
std::vector<std::pair<ValueId, TensorDescriptor>> GetCLNodeTensors(const CLNode &node)
{
  ...
  for (uint32_t j = 0; j < node.outputs.size(); ++j)
  {
...
void AddUsage(ValueId id, int task_index, std::map<ValueId, int2> *usage_records)
{
...
{
  return type == TensorStorageType::BUFFER || type == TensorStorageType::IMAGE_BUFFER;
}
...
bool InferenceContext::InitFromGraph(const CreateInferenceInfo &create_info,
                                     ...
                                     std::vector<uint8_t> *serialized_model)
{
  ...
  // if (ConvertOperations(creation_context.GetDeviceInfo(), graph,
  //                                   create_info.hints) == false) return false;
  if (Merge() == false)
    return false;
  if (AllocateMemory(creation_context.context) == false)
    return false;
  BindMemoryToOperations();
  if (Compile(creation_context) == false)
    return false;
  ...
  //   }
  // }
  // if (Tune(tuning_parameters) == false) return false;
...
// bool InferenceContext::RestoreDeserialized(
...
bool InferenceContext::InitFromGraphWithTransforms(const CreateInferenceInfo &create_info,
                                                   GraphFloat32 *graph, Environment *env,
                                                   std::vector<uint8_t> *serialized_model)
{
  // RETURN_IF_ERROR(RunGraphTransforms(graph));
  if (InitFromGraph(create_info, *graph, env, serialized_model) == false)
    return false;
  return true;
}
...
void InferenceContext::ReserveGraphTensors(const CreateInferenceInfo &create_info,
                                           const DeviceInfo &device_info, const GraphFloat32 &graph)
{
  ValueId max_id = 0;
    ...
    if (graph.IsGraphInput(t->id) || graph.IsGraphOutput(t->id))
    {
      if (shape.c < 4 &&
          CanCreateTensorWithShape(
            device_info, shape,
            TensorDescriptor{data_type, TensorStorageType::SINGLE_TEXTURE_2D, layout}))
      {
        storage_type = TensorStorageType::SINGLE_TEXTURE_2D;
      }
    }
    storage_type = SelectBestStorageType(device_info, shape, storage_type, data_type, layout);
    tensor_reserver_.Add(t->id, {shape, TensorDescriptor{data_type, storage_type, layout}});
    max_id = std::max(max_id, t->id);
  }
  tensor_reserver_.SetNext(max_id + 1);
}

// bool InferenceContext::ConvertOperations(const DeviceInfo& device_info,
...
//       }
//       if (GPUOperationFromNode(device_info, op_def, hints, inputs,
//                                            outputs, node, &gpu_subgraph) == false) return false;
//     }
//     std::unordered_map<int, ValueId> mapping_to_global_ids;
//     for (uint32_t j = 0; j < gpu_subgraph.new_tensors.size(); ++j) {
...
//       for (uint32_t j = 0; j < gpu_op.input_ids.size(); ++j) {
...
//       for (uint32_t j = 0; j < gpu_op.output_ids.size(); ++j) {
                                 ...
                                 std::map<ValueId, int2> *usages)
{
  ...
  for (uint32_t op_index = 0; op_index < nodes_.size(); ++op_index)
  {
...
InferenceContext::TensorMemoryType InferenceContext::GetTensorMemoryType(ValueId id)
{
...
bool InferenceContext::AllocateMemory(CLContext *context)
{
  if (AllocateMemoryForVariableTensors(context) == false)
    return false;
  if (AllocateMemoryForBuffers(context) == false)
    return false;
  if (AllocateMemoryForStrongShapes(context) == false)
    return false;
  return true;
}

bool InferenceContext::AllocateMemoryForVariableTensors(CLContext *context)
{
  ...
  for (auto value_and_ref_value : variable_ids_and_refs_)
  {
        ...
        ref_value_to_tensor_index.end())
    {
      ...
      if (CreateTensor(*context, shape, descriptor,
                       &variable_tensors_[value_and_ref_value.second]) == false)
        return false;
    }
  }
  return true;
...
bool InferenceContext::AllocateMemoryForBuffers(CLContext *context)
{
  std::map<ValueId, int2> buffer_usages;
  GetUsages([this](ValueId id) { return GetTensorMemoryType(id) == TensorMemoryType::BUFFER; },
            &buffer_usages);
  ...
  for (auto &usage : buffer_usages)
  {
    ...
    const auto &shape = t.shape;
    const auto &descriptor = t.descriptor;
    const size_t element_size = descriptor.data_type == DataType::FLOAT32 ? 4 : 2;
    const size_t buffer_size = shape.b * shape.w * shape.h * AlignByN(shape.c, 4) * element_size;
    graph_ids_to_shared_buffer_tensors_[usage.first] = buffer_usage_records.size();
    buffer_usage_records.push_back(
      {buffer_size, static_cast<TaskId>(usage.second.x), static_cast<TaskId>(usage.second.y)});
  }

  ObjectsAssignment<size_t> buffer_assignment;
  if (AssignObjectsToTensors(buffer_usage_records, MemoryStrategy::GREEDY_BEST,
                             &buffer_assignment) == false)
    return false;

  shared_buffers_.resize(buffer_assignment.object_sizes.size());
  for (uint32_t i = 0; i < buffer_assignment.object_sizes.size(); ++i)
  {
    if (CreateReadWriteBuffer(buffer_assignment.object_sizes[i], context, &shared_buffers_[i]) ==
        false)
      return false;
  }
  ...
  for (auto &node : nodes_)
  {
    ...
    for (auto &t : tensors)
    {
      if (GetTensorMemoryType(t.first) != TensorMemoryType::BUFFER)
        continue;
      const int tensor_index = graph_ids_to_shared_buffer_tensors_[t.first];
      if (created_tensors[tensor_index])
        continue;
      const auto &shape = tensor_reserver_.Get(t.first).shape;
      const int buffer_index = buffer_assignment.object_ids[tensor_index];
      if (CreateSharedTensor(*context, shared_buffers_[buffer_index].GetMemoryPtr(), shape,
                             t.second, &shared_buffer_tensors_[tensor_index]) == false)
        return false;
...
bool InferenceContext::AllocateMemoryForStrongShapes(CLContext *context)
{
  std::map<ValueId, int2> usages;
  GetUsages(
    [this](ValueId id) { return GetTensorMemoryType(id) == TensorMemoryType::STRONG_SHAPE; },
    &usages);
  ...
  for (auto &usage : usages)
  {
    ...
    usage_records.push_back({tensor_reserver_.Get(usage.first), static_cast<TaskId>(usage.second.x),
                             static_cast<TaskId>(usage.second.y)});
  }

  ObjectsAssignment<DummyTensor> assignment;
  if (AssignObjectsToTensors(usage_records, MemoryStrategy::EQUALITY, &assignment) == false)
    return false;
  ...
  for (auto &node : nodes_)
  {
    ...
    for (auto &t : tensors)
    {
      ...
      {
        if (CreateTensor(*context, shape, t.second, &strong_shape_tensors_[id]) == false)
          return false;
      }
    }
  }
...
bool InferenceContext::Compile(const CreationContext &creation_context)
{
  for (auto &node : nodes_)
  {
    if (node.operation->Compile(creation_context) == false)
      return false;
  }
  return true;
}

bool InferenceContext::Tune(const TuningParameters &tuning_parameters)
{
  for (auto &node : nodes_)
  {
    if (node.operation->Tune(tuning_parameters) == false)
      return false;
  }
  return true;
}

bool InferenceContext::UpdateParams()
{
  for (auto &node : nodes_)
  {
    if (node.operation->UpdateParams() == false)
      return false;
  }
  return true;
}

bool InferenceContext::AddToQueue(CLCommandQueue *queue)
{
  if (need_manual_release_)
  {
    ...
    if (queue->EnqueueEvent(&prev_enqueue_start_point_) == false)
      return false;
    ;
  }
  int counter = 0;
  for (auto &node : nodes_)
  {
    if (node.operation->AddToQueue(queue) == false)
      return false;
    counter++;
...
// bool InferenceContext::Profile(ProfilingCommandQueue* queue,
//                                        ProfilingInfo* result) {
//   queue->ResetMeasurements();
//   for (auto& node : nodes_) {
//     queue->SetEventsLabel(node.name);
//     if (node.operation->AddToQueue(queue) == false) return false;
//   }
//   if (queue->WaitForCompletion() == false) return false;
//   *result = queue->GetProfilingInfo();
//   return true;
// }

uint64_t InferenceContext::GetSizeOfMemoryAllocatedForIntermediateTensors() const
{
...
// bool InferenceContext::SetInputTensor(ValueId id,
//                                               const TensorFloat32& tensor,
//                                               CLCommandQueue* queue) {
//   return GetTensor(id)->WriteData(queue, tensor);
// }

// bool InferenceContext::GetOutputTensor(ValueId id,
//                                                CLCommandQueue* queue,
