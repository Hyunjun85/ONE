...
bool GPUOperationFromNode(const OperationDef &op_def, const std::vector<Value *> &inputs,
                          const std::vector<Value *> &outputs, OperationType op_type,
                          GPUOperationsSubgraph *gpu_subgraph)
{
  ...
  switch (op_type)
  {
    ...
    {
      if (inputs.size() == 2 && (inputs[0]->tensor.shape.c == inputs[1]->tensor.shape.c ||
      ...
      //   auto attr =
      //       static_cast<ElementwiseAttributes>(node.operation.attributes);
      //   GPUOperation operation =
    ...
    case OperationType::UNKNOWN:
    {
      ...
      //   } else if (inputs.size() == 1 && node.operation.attributes.has_value()) {
      //     auto attr =
      //         std::any_cast<ElementwiseAttributes>(node.operation.attributes);
      //     GPUOperation operation =
      //         CreateElementwise(device_info, op_def, op_type, attr);
      //     *gpu_op = absl::make_unique<GPUOperation>(std::move(operation));
      //     return true;
