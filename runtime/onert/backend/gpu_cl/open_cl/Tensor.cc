...
bool AllocateTensorMemory(const CLContext &context, const BHWDC &shape,
...
bool CreateImageBufferFromBuffer(const CLContext &context, cl_mem memory, DataType data_type,
...
bool CreateTensor(const CLContext &context, const BHWDC &shape, const TensorDescriptor &descriptor,
                  cl_mem memory, Tensor *result)
{
  ...
  {
    CLMemory mem;
    if (AllocateTensorMemory(context, shape, descriptor, nullptr, &mem) == false)
      return false;
    memory = mem.Release();
  }
  ...
  {
    cl_mem image_memory;
    if (CreateImageBufferFromBuffer(context, memory, descriptor.data_type,
                                    shape.b * shape.w * shape.h * shape.d *
                                      DivideRoundUp(shape.c, 4),
                                    &image_memory) == false)
      return false;
...
bool CreateTensorShared(const CLContext &context, const BHWDC &shape,
...
// TODO
bool TensorDescriptor::CreateGPUObject(CLContext *context, GPUObjectPtr *result) const
{
  Tensor gpu_tensor;
  if (gpu_tensor.CreateFromDescriptor(*this, context) == false)
    return false;
...
bool Tensor::GetGPUResources(const GPUObjectDescriptor *obj_ptr,
...
bool Tensor::IsValid(const BHWC &shape) const
{
  if (shape.b != shape_.b)
  {
    return false;
...
bool Tensor::IsValid(const BHWDC &shape) const
{
  if (shape.b != shape_.b)
  {
    return false;
...
// bool Tensor::WriteDataBHWDC(absl::Span<const float> in,
...
// bool Tensor::WriteData(CLCommandQueue* queue,
//                                const TensorFloat32& src) {
//   if (IsValid(src.shape) == false) return false;
//   return WriteDataBHWDC(absl::MakeConstSpan(src.data), queue);
// }

// bool Tensor::WriteData(
//     CLCommandQueue* queue,
...
// bool Tensor::ReadDataBHWDC(absl::Span<float> out,
...
// bool Tensor::ReadData(CLCommandQueue* queue, TensorFloat32* dst) const {
//   if (IsValid(dst->shape) == false) return false;
//   return ReadDataBHWDC(absl::MakeSpan(dst->data), queue);
// }

// bool Tensor::ReadData(CLCommandQueue* queue,
//                               Tensor5DFloat32* dst) const {
//   if (IsValid(dst->shape) == false) return false;
//   return ReadDataBHWDC(absl::MakeSpan(dst->data), queue);
// }

bool Tensor::CreateFromDescriptor(const TensorDescriptor &desc, CLContext *context)
{
  shape_ = desc.shape;
  ...
  if (AllocateTensorMemory(*context, shape_, descriptor_, data_ptr, &memory) == false)
    return false;
  memory_ = memory.Release();
  if (desc.storage_type == TensorStorageType::IMAGE_BUFFER)
  {
    if (CreateImageBufferFromBuffer(*context, memory_, desc.data_type,
                                    shape_.b * shape_.w * shape_.h * shape_.d *
                                      DivideRoundUp(shape_.c, 4),
                                    &image_buffer_memory_) == false)
      return false;
  }
  return true;
}

bool CreateTensor(const CLContext &context, const BHWC &shape, const TensorDescriptor &descriptor,
                  Tensor *result)
{
...
bool CreateSharedTensor(const CLContext &context, cl_mem memory, const BHWC &shape,
...
bool CreateSharedTensor(const CLContext &context, cl_mem memory, const BHWDC &shape,
                        const TensorDescriptor &descriptor, Tensor *result)
{
  return CreateTensorShared(context, shape, descriptor, memory, result);
}

bool AllocateTensorMemory(const CLContext &context, const BHWC &shape,
