...
#ifndef __ONERT_BACKEND_GPU_CL_MODEL_H__
#define __ONERT_BACKEND_GPU_CL_MODEL_H__
...
public:
  ...
  bool InsertNodeAfter(NodeId id, Node **new_node);
  ...
  // output.
  bool DeleteValue(ValueId id);

  bool MakeExactCopy(GraphFloat32 *model) const;
...
private:
  ...
  template <typename T> static void Erase(std::vector<T> *values, T value)
  {
    values->erase(std::find(values->begin(), values->end(), value));
  }

  // @return non-nullptr NodeDef that has valid Node or an error
  bool LookupNode(NodeId id, NodeDef **node_def);

  // @return non-nullptr ValueDef that has valid Value or an error
  bool LookupValue(ValueId id, ValueDef **value_def);

  template <typename Pred> std::vector<Value *> FilterValues(const Pred &predicate) const
  {
  ...
  template <typename Pred> std::vector<Node *> FilterNodes(const Pred &predicate) const
  {
...
#endif // __ONERT_BACKEND_GPU_CL_MODEL_H__
