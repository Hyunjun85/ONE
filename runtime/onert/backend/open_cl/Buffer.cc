...
bool CreateBuffer(size_t size_in_bytes, bool gpu_read_only, const void *data, CLContext *context,
                  Buffer *result)
{
  cl_mem buffer;
  if (CreateCLBuffer(context->context(), size_in_bytes, gpu_read_only, const_cast<void *>(data),
                     &buffer) == false)
  {
    return false;
  }
...
bool BufferDescriptor::CreateGPUObject(CLContext *context, GPUObjectPtr *result) const
{
  Buffer gpu_buffer;
  if (gpu_buffer.CreateFromBufferDescriptor(*this, context) == false)
    return false;
...
bool Buffer::GetGPUResources(const GPUObjectDescriptor *obj_ptr,
...
bool Buffer::CreateFromBufferDescriptor(const BufferDescriptor &desc, CLContext *context)
{
