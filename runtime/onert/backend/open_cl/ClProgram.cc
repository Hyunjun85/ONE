...
// TODO
/*
bool GetProgramBuildInfo(cl_program program, cl_device_id id,
...
*/
bool GetBinarySize(cl_program program, size_t *binary_size)
{
  cl_int error_code =
...
bool BuildProgram(cl_program program, const CLDevice &device, const std::string &compiler_options)
{
  (void)device;
...
bool CLProgram::GetBinary(std::vector<uint8_t> *result) const
{
  size_t binary_size;
  if (!GetBinarySize(program_, &binary_size))
  {
    return false;
  }
...
bool CreateCLProgram(const std::string &code, const std::string &compiler_options,
                     const CLContext &context, const CLDevice &device, CLProgram *result)
{
  ...
  *result = CLProgram(program, device.id());
  if (!BuildProgram(program, device, compiler_options))
  {
    return false;
  }
  return true;
