...
bool GPUOperation::AddOperation(GPUOperation *operation)
{
  linkable_count_ += 1;
  ...
  elementwise_code_ += "{\n" + code + "\n}\n";
  if (args_.Merge(std::move(operation->args_), unique_postfix) == false)
    return false;
  for (u_int32_t i = 0; i < operation->src_tensors_names_.size(); ++i)
  {
  ...
  for (u_int32_t i = 0; i < operation->dst_tensors_names_.size(); ++i)
  {
...
bool GPUOperation::UpdateParams()
{
  for (uint32_t i = 0; i < src_tensors_names_.size(); ++i)
  {
    if (args_.SetObjectRef(src_tensors_names_[i], src_[i]) == false)
      return false;
  }
  for (uint32_t i = 0; i < dst_tensors_names_.size(); ++i)
  {
    if (args_.SetObjectRef(dst_tensors_names_[i], dst_[i]) == false)
      return false;
  }
  if (BindArguments(&args_) == false)
    return false;
...
bool GPUOperation::AssembleCode(const DeviceInfo &device_info, CLContext *context)
{
  if (elementwise_)
  {
    auto src_desc = std::make_unique<TensorDescriptor>(definition_.src_tensors[0]);
    ...
    auto dst_desc = std::make_unique<TensorDescriptor>(definition_.dst_tensors[0]);
    if (definition_.IsBatchSupported())
    {
    ...
    if (args_.AllocateObjects(context) == false)
      return false;
    if (args_.TransformToCLCode(device_info, {{dst_tensors_names_[0], elementwise_code_}},
                                &code_) == false)
      return false;
  }
  else
...
bool GPUOperation::Compile(const CreationContext &creation_context)
{
  if (AssembleCode(creation_context.GetDeviceInfo(), creation_context.context) == false)
    return false;
  if (creation_context.cache->GetOrCreateCLKernel(code_, "main_function", compiler_options_,
                                                  *creation_context.context,
                                                  *creation_context.device, &kernel_) == false)
    return false;
  return PostCompileCheck(creation_context.device->info_, kernel_.info_);
}

bool GPUOperation::CompileDeserialized(const CreationContext &creation_context)
{
...
bool GPUOperation::Tune(const TuningParameters &params)
{
  ...
  else
  {
    ...
    for (u_int32_t i = 0; i < work_groups_count.size(); ++i)
    {
      work_groups_count[i] = GetWorkGroupsCount(grid_dimension_, grid_size_,
                                                possible_work_groups[i], work_group_launch_order_);
    }
    if (args_.Bind(kernel_.kernel()) == false)
      return false;
    int best_work_group_index;
    if (params.queue->GetBestWorkGroupIndex(kernel_, *params.info, work_groups_count,
                                            possible_work_groups, &best_work_group_index) == false)
      return false;
    ;
