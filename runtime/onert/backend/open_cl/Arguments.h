...
public:
  ...
  virtual bool SetHalf(const std::string &name, half value) = 0;
...
public:
  ...
  bool SetHalf(const std::string &name, half value) override;
  bool SetObjectRef(const std::string &name, const GPUObject *object);

  bool Bind(cl_kernel kernel, int offset = 0);

  void RenameArgs(const std::string &postfix, std::string *code) const;
  bool Merge(Arguments &&args, const std::string &postfix);

  bool AllocateObjects(CLContext *context);
  void ReleaseCPURepresentation();
  bool TransformToCLCode(const DeviceInfo &device_info,
...
private:
  ...
  bool SetImage2D(const std::string &name, cl_mem memory);
  bool SetBuffer(const std::string &name, cl_mem memory);
  bool SetImage2DArray(const std::string &name, cl_mem memory);
  bool SetImage3D(const std::string &name, cl_mem memory);
  bool SetImageBuffer(const std::string &name, cl_mem memory);
  bool SetCustomMemory(const std::string &name, cl_mem memory);
  ...
  bool SetGPUResources(const std::string &name, const GPUResourcesWithValue &resources);

  bool AddObjectArgs();

  void ResolveArgsPass(const DeviceInfo &device_info, std::string *code);
  bool ResolveSelectorsPass(const std::map<std::string, std::string> &linkables, std::string *code);

  bool ResolveSelector(const std::map<std::string, std::string> &linkables,
