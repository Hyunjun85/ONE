...
public:
  ...
  bool CreateFromProgram(const CLProgram &program, const std::string &function_name);

  bool SetMemory(int index, cl_mem memory);
  bool SetMemoryAuto(cl_mem memory);
  template <typename T> bool SetBytes(int index, const T &value) const
  {
    return SetBytes(index, static_cast<const void *>(&value), sizeof(T));
  }
  template <typename T> bool SetBytesAuto(const T &value)
  {
...
private:
  void Release();
  bool SetBytes(int index, const void *ptr, int length) const;
  bool SetBytesAuto(const void *ptr, int length);
